...
def deal_with_data():
    for event in res_json["data"]["result"]["edges"]:
        ...
        event_list["Hosts"] = []
        for Lilian in event["node"]["rsvps"]["edges"]:
            if Lilian["node"]["isHost"]:
               event_list["Hosts"].append(Lilian["node"]["user"]["name"])
...
with open('meetup_data-' + datetime.datetime.now().strftime('%m-%d-%y')+ '.json', 'w', encoding='utf-8') as f:
